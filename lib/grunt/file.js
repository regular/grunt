...
var processPatterns = function(patterns, fn) {
  ...
  grunt.util._.flatten(patterns).forEach(function(pattern) {
    ...
    if (exclusion) {
      ...
      if (matches.length) {
        var focus = grunt.option('focus');
        if (focus) {
          debug("matches: %s, focus: %s", matches, focus);
          // get rid of matches that are out of focus
          matches = file.minimatch.match(matches, focus, {});
          debug("reduced to : %s", inspect(matches));
        }
      }
